...
for l in range (1,(loop+1)):     # Loop para a analise das Projecoes
    ...
    if (l == 2):
       ...
       pdos_u_Px = dos_up[:,9];   pdos_d_Px = dos_down[:,9]
       pdos_u_Py = dos_up[:,10];  pdos_d_Py = dos_down[:,10]
       pdos_u_Pz = dos_up[:,11];  pdos_d_Pz = dos_down[:,11]
       #----------------------------------------------------
       x_final   = max(dos_u_tot)
       x_inicial = min(dos_d_tot)         
       #===============================================================================================================
    ...
    if (l == 3):
       ...
       pdos_u_Dxy = dos_up[:,12];  pdos_d_Dxy = dos_down[:,12]
       pdos_u_Dyz = dos_up[:,13];  pdos_d_Dyz = dos_down[:,13]
       pdos_u_Dz2 = dos_up[:,14];  pdos_d_Dz2 = dos_down[:,14]
       pdos_u_Dxz = dos_up[:,15];  pdos_d_Dxz = dos_down[:,15]
       pdos_u_Dx2 = dos_up[:,16];  pdos_d_Dx2 = dos_down[:,16]
       #------------------------------------------------------
       x_final   = max(dos_u_tot)
       x_inicial = min(dos_d_tot)
       #===============================================================================================================
...
print(" ")
print("====== Plotando a Magnetizacao (DOS_Up - DOS_Down) ======")
...
for l in range (1,(loop+1)):     # Loop para a analise das Projecoes
    ...
    if (l == 1):
       print ("===================================")
       print ("Analisando a Magnetizacao (S, P, D)")
       #--------------------------------------------
    ...
    if (l == 2):
       print ("Analisando a Magnetizacao (Px, Py, Pz)")       
       #-----------------------------------------------
       pdos_P  = dos_up[:,5]  + dos_down[:,5]
       ldos_P  = dos_up[:,6]  + dos_down[:,6]       
       pdos_Px = dos_up[:,9]  + dos_down[:,9]
       pdos_Py = dos_up[:,10] + dos_down[:,10]
       pdos_Pz = dos_up[:,11] + dos_down[:,11]
       #-----------------------       
       x_final   = max(pdos_P)
    ...
    if (l == 3):
       print ("Analisando a Magnetizacao (Dxy, Dyz, Dz2, Dxz, Dx2)") 
       #------------------------------------------------------------
       pdos_D   = dos_up[:,7]  + dos_down[:,7]       
       ldos_D   = dos_up[:,8]  + dos_down[:,8]
       pdos_Dxy = dos_up[:,12] + dos_down[:,12]
       pdos_Dyz = dos_up[:,13] + dos_down[:,13]
       pdos_Dz2 = dos_up[:,14] + dos_down[:,14]
       pdos_Dxz = dos_up[:,15] + dos_down[:,15]
       pdos_Dx2 = dos_up[:,16] + dos_down[:,16]
       #-----------------------       
       x_final   = max(pdos_D)
    ...
    plt.xlabel("Magnetizacao (DOS_Up - DOS_Down)")

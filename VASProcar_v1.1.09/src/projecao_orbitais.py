...
if (escolha == -1):
   ...
   if (esc_color == 1):
      ...
      print ("                                                              ")
      print ("Cor dos Orbitais S|P|D:  4 2 3  (Blue, Red, Green)            ")
      print ("--------------------------------------------------------------")      
      print ("Cor dos Orbitais Px|Py|Pz:  4 2 3  (Blue, Red, Green)         ")
      print ("--------------------------------------------------------------") 
      print ("Cor dos Orbitais Dxy|Dyz|Dz2|Dxz|Dx2: 4 2 3 10 9              ")
      print ("(Blue, Red, Green, Magenta, Cyan)                             ")
      print ("##############################################################") 
      print (" ")

      print ("==============================================================") 
      print ("Digite em sequencia as cores dos orbitais S|P|D:              ")
...
#======================================================================
# Organizando as contribuicoes para cada tipo de Orbital ============== # ????????????????????????????????????????????????????????????????????????
#======================================================================
            ...
            for point_k in range (1, (nk+1)):
                for Band_n in range (1, (nb+1)):
                    ...
                    if (j == 7): # Orbital Dxy 
                       ...
                       if (DFT == '_QE/'):   orb_Dxy[wp][point_k][Band_n] = soma_orb[wp][9][point_k][Band_n]
                    if (j == 8): # Orbital Dyz 
                       if (DFT == '_VASP/'): orb_Dyz[wp][point_k][Band_n] = soma_orb[wp][6][point_k][Band_n]
                       if (DFT == '_QE/'):   orb_Dyz[wp][point_k][Band_n] = soma_orb[wp][7][point_k][Band_n]
                    if (j == 9): # Orbital Dz2 
                       if (DFT == '_VASP/'): orb_Dz2[wp][point_k][Band_n] = soma_orb[wp][7][point_k][Band_n]
                       if (DFT == '_QE/'):   orb_Dz2[wp][point_k][Band_n] = soma_orb[wp][5][point_k][Band_n]
                    if (j == 10): # Orbital Dxz 
                       if (DFT == '_VASP/'): orb_Dxz[wp][point_k][Band_n] = soma_orb[wp][8][point_k][Band_n]
                       if (DFT == '_QE/'):   orb_Dxz[wp][point_k][Band_n] = soma_orb[wp][6][point_k][Band_n]
                    if (j == 11): # Orbital Dx2 
                       if (DFT == '_VASP/'): orb_Dx2[wp][point_k][Band_n] = soma_orb[wp][9][point_k][Band_n]
                       if (DFT == '_QE/'):   orb_Dx2[wp][point_k][Band_n] = soma_orb[wp][8][point_k][Band_n]
...
linha += 1; lines.insert(linha, f'c_S = {c_S}; c_P = {c_P}; c_D = {c_D} \n')
if (lorbit >= 11):

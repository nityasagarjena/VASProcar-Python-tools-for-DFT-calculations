...
for i in range(nb + 1 + contador2):
    if ((i < nb and bands_sn[i+1] == "sim") or (i >= nb)):
       n_band += 1
 
       if (ispin == 1):
          if (i < nb):  color = 1 # color Black
          if (i == nb): color = 2 # color Red

       if (ispin == 2):
          if (i < (nb/2)):
             color = 2 # color Red
             legenda = 'Spin' + ' \\f{Symbol}\\c-\\C'
             n_s1 += 1
          if (i >= (nb/2) and i < nb):
             color = 4 # color Blue
             legenda = 'Spin' + ' \\f{Symbol}\\c/\\C'
       ...
       bandas.write(f'@    s{n_band} type xy \n')
       bandas.write(f'@    s{n_band} line type 1 \n')
       bandas.write(f'@    s{n_band} line color {color} \n')
    
       if (ispin == 2 and n_s1 == 1):  bandas.write(f'@    s{n_band} legend  "{legenda}" \n') 
       if (ispin == 2 and n_s2 == 1):  bandas.write(f'@    s{n_band} legend  "{legenda}" \n')

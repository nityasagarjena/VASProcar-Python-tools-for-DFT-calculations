...
#===========================================================================
# Plot das Projeções dos Orbitais de forma individual: =====================
#===========================================================================
...
for l in range (1,(loop+1)):     # Loop para a analise das Projecoes
    ...
    if (l == 1):
       ...
       Orb_tot = S+P+D
       #---------------------------
       pS = ((dpi*S)**2)*peso_total
       pP = ((dpi*P)**2)*peso_total
       pD = ((dpi*D)**2)*peso_total
       pOrb_tot = ((dpi*Orb_tot)**2)*peso_total
       #---------------------------------------
    ...
    if (l == 2):
       #--------------------------------------------------
       Px = orbitais[:,5];  pPx = ((dpi*Px)**2)*peso_total
       Py = orbitais[:,6];  pPy = ((dpi*Py)**2)*peso_total
       Pz = orbitais[:,7];  pPz = ((dpi*Pz)**2)*peso_total
       #--------------------------------------------------
    ...
    if (l == 3):
       Dxy = orbitais[:,8];   pDxy = ((dpi*Dxy)**2)*peso_total
       Dyz = orbitais[:,9];   pDyz = ((dpi*Dyz)**2)*peso_total
       Dz2 = orbitais[:,10];  pDz2 = ((dpi*Dz2)**2)*peso_total
       Dxz = orbitais[:,11];  pDxz = ((dpi*Dxz)**2)*peso_total
       Dx2 = orbitais[:,12];  pDx2 = ((dpi*Dx2)**2)*peso_total
       #------------------------------------------------------      
    ...
    if (l == 1): print ("Plot da projecao dos orbitais S|P|D concluido -----------------")
    if (l == 2): print ("Plot da projecao dos orbitais Px|Py|Pz concluido --------------")
    if (l == 3): print ("Plot da projecao dos orbitais Dxy|Dyz|Dz2|Dxz|Dx2 concluido ---")

#==============================================================================================================
# Obtendo e gravando as cores no padrão RGB que designam a combinação de orbitais para o Plot das Projeções ===
#==============================================================================================================

print ("Analisando a sobreposicao de Orbitais (Soma do padrao de cores)")

# Inicialização de Variaveis, Vetores e Matrizes a serem utilizadas -------
   
rgb_SPD = [0]*n_procar*nk*nb
rgb_P   = [0]*n_procar*nk*nb
rgb_D   = [0]*n_procar*nk*nb
...
for Band_n in range (1, (nb+1)):
    for wp in range (1, (n_procar+1)):
        for point_k in range (1, (nk+1)):       
            ...
            #--------------------------------------------------------------------------------------------------
            # Somando as cores dos orbitais (S,P,D): ----------------------------------------------------------
            #--------------------------------------------------------------------------------------------------

            color_dat = [0]*3
           
            for i in range(3):
                color_dat[i]  = S[number]*cRGB[c_S][i] + P[number]*cRGB[c_P][i] + D[number]*cRGB[c_D][i]
                #-------------------------------------------
                if (color_dat[i]  > 1.0): color_dat[i] = 1.0
                #-------------------------------------------
            rgb_SPD[number] = (color_dat[0], color_dat[1], color_dat[2])    

            #---------------------------------------------------------------------------------------------------
            # Somando as cores dos orbitais (Px,Py,Pz): --------------------------------------------------------
            #---------------------------------------------------------------------------------------------------

            color_dat = [0]*3

            if (P[number] != 0 and lorbit >= 11):
            ...
            #---------------------------------------------------------------------------------------------------
            # Somando as cores dos orbitais (Dxy,Dyz,Dz2,Dxz,Dx2): ---------------------------------------------
            #---------------------------------------------------------------------------------------------------

            color_dat = [0]*3

            if (D[number] != 0 and lorbit >= 11):
...
for l in range (1,(loop+1)):     # Loop para a analise das Projecoes
    ...
    if (l == 1):
       ...
       file = 'Orbitais_SPD'
       #----------------------
       ax.scatter(rx, ry, s = peso, c = rgb_SPD, alpha = transp, edgecolors = 'none')            
       #----------------------
       # Gambiarra: A fim de gerar uma legenda cuja esfera colorida possua um tamanho fixo (antes ela adotada o menor ou maior (???) tamanho de esfera do plot)
       #            e sem a transparência do plot, tive que plotar pontos isolados afastados da região do gráfico.
       ax.scatter([-1000.0], [-1000.0], s = [40.0], color = cRGB[c_S], alpha = 1.0, edgecolors = 'none', label = 'S')

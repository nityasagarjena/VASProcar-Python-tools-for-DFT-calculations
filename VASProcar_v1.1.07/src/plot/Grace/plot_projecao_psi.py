...
for j in range (1,(n_psi+1)):
    ...
    legend = l_psi[j-1]; color = cor_orb[j+6]  #  Cor do estado Psi
    grac = 's' + str(j-1)
    ...
    for wp in range (1,(n_procar+1)):
        for point_k in range (1,(nk+1)):
            for Band_n in range (1,(nb+1)):        
                ...
                if (psi[j][wp][point_k][Band_n] > 0.0):    
                   projection.write(f'{xx[wp][point_k]} {Energia[wp][point_k][Band_n] + dE_fermi} {psi[j][wp][point_k][Band_n]*peso_total} \n')
                #------------------------------------------------------------------------------------------------------------------------------  
    projection.write(" \n")
